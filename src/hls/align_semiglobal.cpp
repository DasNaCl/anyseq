...
#pragma HLS LOOP_FLATTEN
      if(degenerate[pe]) {
        ...
        if(pe + 1 < pe_count) {
          ...
          inputs[pe + 1][IMAXI_1J_IDX] = scoring[pe][IMAXI_1J_IDX];
        }
        continue;
      }
			...
			const ScoreType hij = max(m1, m2);

			const ScoreType maxij = max((i - pe >= subject_len - 1 ? hij : -32768), inputs[pe][IMAXI_1J_IDX]);
			// update my data
